...
const views = {
    ...
    admin_board :  path.join(__dirname, '../../', 'public/views/admin_board.html'),
    create_room : path.join(__dirname, '../../', 'public/views/create_room.html')
};
...
app.get(['/admin_board'], (req, res) => {
    res.sendFile(views.admin_board);
});

///

app.get(['/create_room'], (req, res) => {
    res.sendFile(views.create_room);
});
// mirotalk about
...
app.post([apiBasePath + '/meeting'], (req, res) => {
    ...
    if (authorization != api_key_secret) {
        log.debug('VideoConference get meeting - Unauthorized', {
            header: req.headers,
            body: req.body,
        });
    ...
    log.debug('VideoConference get meeting - Authorized', {
        header: req.headers,
        body: req.body,
        meeting: meetingURL,
    });
